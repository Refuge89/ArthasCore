...
const char * Transmogrification::GetSlotName(uint8 slot, WorldSession* session) const
{
    switch (slot)
    {
    case EQUIPMENT_SLOT_HEAD      : return  "Галава";// session->GetTrinityString(LANG_SLOT_NAME_HEAD);
    ...
    case EQUIPMENT_SLOT_WAIST     : return  "Талия";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
    ...
    case EQUIPMENT_SLOT_BACK      : return  "Назад";// session->GetTrinityString(LANG_SLOT_NAME_BACK);
...
TransmogTrinityStrings Transmogrification::Transmogrify(Player* player, uint64 itemGUID, uint8 slot, /*uint32 newEntry, */bool no_cost)
{
    ...
    if (slot >= EQUIPMENT_SLOT_END)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an item (lowguid: %u) with a wrong slot (%u) when transmogrifying items.", player->GetGUIDLow(), player->GetName().c_str(), GUID_LOPART(itemGUID), slot);
    ...
    */
    ...
    if (itemGUID)
    {
        ...
        {
			sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (lowguid: %u).", player->GetGUIDLow(), player->GetName().c_str(), GUID_LOPART(itemGUID));
    ...
    {
		sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an invalid item in a valid slot (slot: %u).", player->GetGUIDLow(), player->GetName().c_str(), slot);
